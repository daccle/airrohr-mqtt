...
One way to persist the execution, is to start with systemd in the context of the current user. 
An example configuration is provided in `airrohr-mqtt.service.EXAMPLE`. You want to copy this
file to `~/.config/systemd/user/airrohr-mqtt.service`. Enable it with 
`systemctl --user enable airrohr-mqtt.service`, start with 
`systemctl --user start airrohr-mqtt.service`.
Console output can be observed by `journalctl --user-unit airrohr-mqtt.service`. In order to 
make it run, even when the user is not logged in, make sure to enable lingering. You can use
`loginctl`like so: `loginctl enable-linger`
